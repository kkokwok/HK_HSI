...
Signal Toggles: Check or uncheck Show Peak Signals and Show Bottom Signals to display or hide automated reversal markers (Warning triangles and Confirmed circles).
Auto Refresh Rate: Set the timer (in seconds) to automatically fetch live updates from Yahoo Finance.
Main Interactive Chart (Right Panel):
...
訊號開關：勾選或取消勾選「顯示見頂訊號」與「顯示見底訊號」，以顯示或隱藏自動反轉標記（警告三角形與確認圓圈）。
...
面板 2（正常K線）：展示標準的原始開高低收 (OHLC) K線圖及對應的傳統移動平均線。
面板 3、4、5（副圖指標）：透過 MACD、RSI(14)（附帶 70/30 虛線超買超賣參考線）以及 KDJ 震盪指標（K, D, J 線）追蹤市場動能與超買超賣狀態。
縮放與平移：在任意圖表面板上按住並拖曳即可放大局部；在面板任意處雙擊即可重置檢視視圖。各面板的 Y 軸均可獨立自動縮放。
